<template>
  <div class="chat-container">
    <div class="page-header">
      <h2>客服管理</h2>
      <p class="subtitle">与客户实时沟通，提供优质服务</p>
    </div>

    <el-row :gutter="0" class="chat-main">
      <!-- 左侧客户列表 -->
      <el-col :span="6" class="session-list">
        <div class="session-header">
          <h3>客户列表</h3>
          <el-badge :value="totalUnread" :hidden="totalUnread === 0" class="total-unread" />
        </div>
        <div class="session-items">
          <div v-if="sessions.length === 0" class="empty-sessions">
            <el-empty description="暂无客户咨询" />
          </div>
          <div 
          ...
          >
            <el-avatar :src="session.customerAvatar || '/default-avatar.png'" :size="45" />
            <div class="session-info">
              <div class="customer-name">{{ session.customerName }}</div>
      ...
      <el-col :span="18" class="chat-window">
        <div v-if="currentSession" class="chat-content">
          <div class="chat-header">
            <div class="header-info">
              <el-avatar :src="currentSession.customerAvatar || '/default-avatar.png'" :size="40" />
              <div>
                <h3>{{ currentSession.customerName }}</h3>
                <p class="customer-id">客户ID: {{ currentSession.customerId }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="message-list" ref="messageContainer" v-loading="messageLoading">
            <div 
              ...
              :key="msg.id"
              class="message-item"
              :class="{ 'is-mine': msg.senderType === 2 }"
            >
              <el-avatar 
          ...
          <div class="message-input">
            <div class="input-toolbar">
              <el-dropdown @command="handleQuickReply" trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item command="您好，欢迎光临本店！有什么可以帮您的吗？">欢迎语</el-dropdown-item>
                    <el-dropdown-item command="您可以在订单详情页查看物流信息哦~">查询物流</el-dropdown-item>
                    <el-dropdown-item command="如需退换货，请在订单详情页申请售后服务。">退换货</el-dropdown-item>
                    <el-dropdown-item command="感谢您的咨询，祝您购物愉快！">感谢语</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </div>
        ...
        <div v-else class="empty-chat">
          <el-empty description="选择一个客户开始聊天" />
        </div>
      </el-col>
    </el-row>
  </div>
...
const totalUnread = computed(() => {
  return sessions.value.reduce((sum, session) => sum + (session.unreadCount || 0), 0)
})
